...
public:
	...
	bool_t setParity(usartParity_e parity);
	bool_t setMode(usartMode_e mode);
	bool_t setBaudRate(usartBaudRate_e baudRate);
	bool_t setDataBits(usartDataBits_e dataBits);
	bool_t setStopBit(usartStopBit_e stopBit);
...
bool_t inline __attribute__((always_inline)) Usart0::isReceptionComplete(void)
{
	return isBitSet(UCSR0A, RXC0);
}

bool_t inline __attribute__((always_inline)) Usart0::isTransmissionComplete(void)
{
	return isBitSet(UCSR0A, TXC0);
}

bool_t inline __attribute__((always_inline)) Usart0::isBufferEmpty(void)
{

...
void configPeripherals(void)
{
	...
	if (halStatus != HAL_OK) {
		errorCode = ERROR_CODE_ADC_CALIBRATION_FAILED;
		Error_Handler();
	}
	halStatus = HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adcDmaBuffer, ADC_DMA_BUFFER_ELEMENTS);
	if (halStatus != HAL_OK) {
		errorCode = ERROR_CODE_ADC_START_DMA_FAILED;
		Error_Handler();
	}
	...
	if (halStatus != HAL_OK) {
		errorCode = ERROR_CODE_TIMER_BASETIME_START_INTERRUPT_FAILED;
		Error_Handler();
	}

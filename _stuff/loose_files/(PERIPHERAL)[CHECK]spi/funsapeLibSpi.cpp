...
bool Spi::writeReg(uint8_t reg_p, uint8_t *buffData_p, uint16_t buffSize_p)
{
	...
	if((!this->_devSelFunctionSet) || (!this->_devSelPinSet)) {
		// Return error
...
bool Spi::setDevice(GPIO_TypeDef *csPort_p, uint16_t csPin_p)
{
	// Clear data members
	this->_devSelFunctionSet = false;
	this->_devSelPinSet = false;
	...
	// CHECK FOR ERROR - transmission buffer data is null pointer
	if(!isPointerValid(csPort_p)) {				// Testing csPort_p parameter
		// Returns error
	...
	// Update data members
	this->_csPin = csPin_p;
	this->_csPort = csPort_p;
	this->_devSelPinSet = true;
...
#if defined(_FUNSAPE_PLATFORM_ARM_STM32) && defined(HAL_SPI_MODULE_ENABLE)
	do {
...
bool Spi::_transmit(SpiOperation spiOperation_p)
{
	...
	} else if(this->_devSelPinSet) {			// Select device using pin
	...
	} else if(this->_devSelPinSet) {			// Deselect device using pin

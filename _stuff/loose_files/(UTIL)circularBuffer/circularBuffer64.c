 ...
 * Module:			Circular Buffer Implementation for 64 bits variables
...
bool_t circularBuffer64Init(circularBuffer64_t * buffer, uint16_t bufferSize)
{
	...
	// Memory allocation
	buffer->data = (uint64_t *)malloc(bufferSize * sizeof(uint64_t));
	if(buffer->data == NULL) {
		return FALSE;
	}
 ...
 * internal pointer will not be changed. The circularBuffer64IsEmpty() function
 ...
 * by circularBuffer64PushData() calls.
 * -------------------------------------------------------------------------- */

 ...
 * Module:		16-bits data-wide circular buffer for GPDSE++ ARM Library project
...
#if __CIRCULAR_BUFFER_16_H != __BUILD_MACRO_TO_BE_CHANGED__
#	error Build mismatch on header and source code files (circularBuffer16).
#endif
...
CircularBuffer16::CircularBuffer16(uint16_t bufferSize, bool overwrite)
{
	...
	// Allocate memory
	this->data_ = (uint16_t *)calloc(bufferSize, sizeof(uint16_t));
...
bool inline __attribute__((always_inline)) CircularBuffer16::checkLocked_(void)
{
	if (this->locked_) {
...
bool inline __attribute__((always_inline)) CircularBuffer16::checkInitialized_(void)
{
...
bool inline __attribute__((always_inline)) CircularBuffer16::checkWriteProtected_(void)
{
...
bool inline __attribute__((always_inline)) CircularBuffer16::checkReadProtected_(void)
{
...
void inline __attribute__((always_inline)) CircularBuffer16::moveReadPointer_(void)
{
...
void inline __attribute__((always_inline)) CircularBuffer16::moveWritePointer_(void)
{
...
void inline __attribute__((always_inline)) CircularBuffer16::moveBothPointers_(void)
{
	this->wrIndex_++;							// Increments write pointer
	this->wrIndex_ %= this->maxSize_;			// Resolves write pointer overflow
	this->rdIndex_++;							// Increments read pointer

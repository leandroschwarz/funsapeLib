...
Mpu9250::Mpu9250(Handler *hdlr)
{
    // Reset data members
    this->_resetDataMembers();

    // Check for errors - Handler pointer
    if (!isPointerValid(hdlr)) {
...
bool Mpu9250::setHandler(Handler *hdlr)
{
    // Check for errors - Handler null pointer
    if (!hdlr) {
        this->_lastError = ERROR_HANDLER_POINTER_NULL;
        return false;
    }
...
bool Mpu9250::init(void)
{
    ...
    if ((this->_twiSpi) && (!this->_callbacksConfig)) {
        this->_lastError = ERROR_COMMUNICATION_NO_DEVICE_SELECTED;
        return false;
    }

    // If SPI, change default device
    if (this->_twiSpi) {                // SPI interface mode
        this->_handler->setDevice(this->_activateDevice, this->_deactivateDevice);
    }

    // Checking WHO_AM_I register
    auxBuff[0] = REG_WHO_AM_I | SPI_READ;
    auxBuff[1] = SPI_DUMMY_BYTE;
    this->_handler->sendData(auxBuff, 2);
    if (auxBuff[1] != MPU9250_WHO_AM_I) {
...
bool Mpu9250::setDevice(void (* actFunc)(void), void (* deactFunc)(void))
{
    // Check for errors - Function pointers
    if ((actFunc == nullptr) || (deactFunc == nullptr)) {
...
bool Mpu9250::configInterrupts(void)
{

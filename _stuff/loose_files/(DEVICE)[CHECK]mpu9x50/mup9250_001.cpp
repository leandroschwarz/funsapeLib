...
Mpu9250::Mpu9250(void)
{
	...
	this->_accelDlpfCfg = MPU9250_ACCEL_DLPF_218_HZ;
	this->_accelFChoiceB = false;
	this->_accelFullScale = MPU9250_AFS_2G;
	this->_sampleRateDivider = 0;
	this->_gyroDisableX = false;
	this->_gyroDisableY = false;
	this->_gyroDisableZ = false;
	this->_gyroDlpfCfg = MPU9250_GYRO_DLPF_250_HZ;
	this->_gyroFChoiceB = 0;
	this->_gyroFullScale = MPU9250_GFS_250DPS;
...
bool Mpu9250::initialize(Twi0 *i2cHndlr, bool useAlternateAddress)
{
	// Check for errors
...
bool Mpu9250::initialize(Spi0 *spiHndlr, void (*actDevFuncCallback)(void), void (*deactDevFuncCallback)(void))
{
	// Check for errors
...
bool Mpu9250::configIntPin(logic_t pinLevel,
		driverMode_t pinDriver,
		bool pinLatch,
		bool pinAutoReset)
{
	// Check for errors
...
bool Mpu9250::_getWhoAmI(void)
{
	...
	if(whoAmI != MPU9250_WHO_AM_I) {
		this->_lastError = ERROR_DEVICE_ID_MATCH_FAILED;
		return false;
	}

...
// MY FUNCTIONS
uint8_t twiStrangeGolbalBuff[20];
...
{
	twiStrangeGolbalBuff[0] = reg_addr;
	for(uint8_t i = 0; i < length; i++) {
		twiStrangeGolbalBuff[i + 1] = data[i];
	}
	twiMasterSendData(slave_addr, TWI_WRITE, twiStrangeGolbalBuff, length + 1);
...
bool_t i2c_read(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data)
{
	...
	twiMasterSendData(slave_addr, TWI_WRITE, twiStrangeGolbalBuff, 1);
	twiMasterSendData(slave_addr, TWI_READ, twiStrangeGolbalBuff, length);
	for(uint8_t i = 0; i < length; i++) {
		data[i] = twiStrangeGolbalBuff[i];
	}
	return FALSE;			// INVERSE LOGIC
}
...
struct msp430_int_s {
	void (*p1_cbs[8])(void);
	void (*p2_cbs[8])(void);
	unsigned char p1_exit[8];
	unsigned char p2_exit[8];
...
int set_int_enable(unsigned char enable)
{
	...
	if(st.chip_cfg.dmp_on) {
		...
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_INT_ENABLE, 1, &tmp)) {
			return -1;
		}
...
// 		if(ii == MPU9250_REGISTER_FIFO_R_W || ii == st.reg->mem_r_w) {
// 			continue;
// 		}
...
// {
// 	if(reg == MPU9250_REGISTER_FIFO_R_W || reg == st.reg->mem_r_w) {
// 		return -1;
// 	}
...
int mpu_init(struct int_param_s * int_param)
{
	...
	/* Reset device. */
	data[0] = BIT_RESET;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_PWR_MGMT_1, 1, data)) {
		return -1;
	}
	delay_ms(100);
...
int mpu_lp_accel_mode(unsigned short rate)
{
	...
	if(!rate) {
		...
		tmp[0] = 0;
		tmp[1] = BIT_STBY_XYZG;
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_PWR_MGMT_1, 2, tmp)) {
			return -1;
		}
		st.chip_cfg.lp_accel_mode = 0;
		return 0;
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_LP_ACCEL_ODR, 1, tmp)) {
		return -1;
	}
	/* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_ACCEL_CONFIG_2, 1, tmp)) {
		return -1;
	}

	tmp[0] = BIT_ACCL_FC_B | tmp[0];
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_ACCEL_CONFIG_2, 1, tmp)) {
		return -1;
	}
	tmp[0] = BIT_LPA_CYCLE;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_PWR_MGMT_1, 1, tmp)) {
		return -1;
	}
...
int mpu_get_gyro_reg(short * data, unsigned long * timestamp)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_GYRO_XOUT_H, 6, tmp)) {
		return -1;
	}
...
int mpu_get_accel_reg(short * data, unsigned long * timestamp)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_ACCEL_XOUT_H, 6, tmp)) {
		return -1;
	}
	data[0] = (tmp[0] << 8) | tmp[1];
	data[1] = (tmp[2] << 8) | tmp[3];
...
int mpu_get_temperature(long * data, unsigned long * timestamp)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_TEMP_OUT_H, 2, tmp)) {
		return -1;
	}
	raw = (tmp[0] << 8) | tmp[1];
	if(timestamp) {
...
int mpu_reset_fifo(void)
{
	...
	data = 0;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_INT_ENABLE, 1, &data)) {
		return -1;
	}
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_FIFO_EN, 1, &data)) {
		return -1;
	}
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, &data)) {
		return -1;
	}
	...
	if(st.chip_cfg.dmp_on) {
		...
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_FIFO_EN, 1, &st.chip_cfg.fifo_enable)) {
			return -1;
		}
	}
	return 0;
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_GYRO_CONFIG, 1, &data)) {
		return -1;
	}
...
int mpu_set_accel_fsr(unsigned char fsr)
{
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_ACCEL_CONFIG, 1, &data)) {
		return -1;
	}
...
int mpu_set_sample_rate(unsigned short rate)
{
	...
	if(st.chip_cfg.dmp_on) {
		...
		data = 1000 / rate - 1;
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_SMPLRT_DIV, 1, &data)) {
			return -1;
		}
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV4_CTRL, 1, &div)) {
		return -1;
	}
...
int mpu_set_sensors(unsigned char sensors)
{
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_PWR_MGMT_2, 1, &data)) {
		st.chip_cfg.sensors = 0;
		return -1;
	}
...
#else
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, &user_ctrl)) {
		return -1;
	}
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV0_DO, 1, &data)) {
		return -1;
	}
	/* Enable/disable I2C master mode. */
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, &user_ctrl)) {
		return -1;
	}
#endif
...
int mpu_get_int_status(short * status)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_INT_STATUS, 2, tmp)) {
		return -1;
	}
                  ...
                  unsigned char *sensors, unsigned char *more)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_COUNT_H, 2, data)) {
		return -1;
	}
...
//    log_i("FIFO count: %hd\n", fifo_count);
	if(fifo_count > (st.hw->max_fifo >> 1)) {
		...
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_INT_STATUS, 1, data)) {
			return -1;
		}
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_R_W, packet_size, data)) {
		return -1;
	}
                         ...
                         unsigned char *more)
{
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_COUNT_H, 2, tmp)) {
		return -1;
	}
	...
	if(fifo_count > (st.hw->max_fifo >> 1)) {
		/* FIFO is 50% full, better check overflow bit. */
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_INT_STATUS, 1, tmp)) {
			return -1;
		}
	...
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_R_W, length, data)) {
		return -1;
	}
...
int mpu_set_bypass(unsigned char bypass_on)
{
	...
	if(bypass_on) {
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, &tmp)) {
			return -1;
		}
		tmp &= ~BIT_AUX_IF_EN;
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, &tmp)) {
			return -1;
		}
		delay_ms(3);
		tmp = BIT_BYPASS_EN;
		...
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_INT_PIN_CFG, 1, &tmp)) {
			return -1;
		}
		_delay_ms(1);
	} else {
...
int get_st_biases(long * gyro, long * accel, unsigned char hw_test)
{
	...
	data[0] = 0x01;
	data[1] = 0;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_PWR_MGMT_1, 2, data)) {
		return -1;
	}
	delay_ms(200);
	data[0] = 0;
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_MST_STATUS, 1, data)) {
		return -1;
	}
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 1, data)) {
		return -1;
	...
	for(ii = 0; ii < packet_count; ii++) {
		...
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_R_W, MAX_PACKET_LENGTH, data)) {
			return -1;
		}
...
const unsigned short mpu_6500_st_tb[256] = {
...
int get_st_6500_biases(long * gyro, long * accel, unsigned char hw_test, int debug)
{
	...
	while(s < test.packet_thresh) {
		...
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_FIFO_R_W, read_size, data)) {
			return -1;
		}
		ind = 0;
		for(ii = 0; ii < packet_count; ii++) {
...
//	for(akm_addr = 0x0C; akm_addr <= 0x0F; akm_addr++) {
	...
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV0_ADDR, 1, data)) {
		return -1;
	}

	/* Compass reads start at this register. */
	data[0] = AKM_REG_ST1;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV0_REG, 1, data)) {
		return -1;
	}

	/* Enable slave 0, 8-byte reads. */
	data[0] = BIT_SLAVE_EN | 8;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV0_CTRL, 1, data)) {
		return -1;
	}

	/* Slave 1 changes AKM measurement mode. */
	data[0] = st.chip_cfg.compass_addr;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV1_ADDR, 1, data)) {
		return -1;
	}

	/* AKM measurement mode register. */
	data[0] = AKM_REG_CNTL;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV1_REG, 1, data)) {
		return -1;
	}

	/* Enable slave 1, 1-byte writes. */
	data[0] = BIT_SLAVE_EN | 1;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_SLV1_CTRL, 1, data)) {
		return -1;
	}
	...
	data[0] = 0x03;
	if(i2c_write(st.hw->addr, MPU9250_REGISTER_I2C_MST_DELAY_CTRL, 1, data)) {
		return -1;
	}
...
#else
	if(i2c_read(st.hw->addr, MPU9250_REGISTER_EXT_SENS_DATA_00, 8, tmp)) {
		return -1;
	}
#endif
                            ...
                            unsigned short lpa_freq)
{
	...
	if(lpa_freq) {
		...
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_USER_CTRL, 3, data)) {
			goto lp_int_restore;
		}

		/* Set motion threshold. */
		data[0] = thresh_hw;
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_WOM_THR, 1, data)) {
			goto lp_int_restore;
		}
		...
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_LP_ACCEL_ODR, 1, data)) {
			goto lp_int_restore;
		}

		/* Enable motion interrupt (MPU6500 version). */
		data[0] = BITS_WOM_EN;
		if(i2c_write(st.hw->addr, MPU9250_REGISTER_MOT_DETECT_CTRL, 1, data)) {
			goto lp_int_restore;
		}
		/* Enable LP ACCEL mode, ACCEL_FCHOICE_B=1*/
		if(i2c_read(st.hw->addr, MPU9250_REGISTER_ACCEL_CONFIG_2, 1, temp)) {
			return -1;
		}

...
static void lcdTwiFunctionSet8Bits(lcdTwi_t *lcd, uint8_t command);
static void lcdTwiSendData(lcdTwi_t *lcd, uint8_t data, lcdTwiDataType_t type);
static int16_t lcdTwiWriteStd(int8_t c, FILE *stream);

// -----------------------------------------------------------------------------
// Global variables ------------------------------------------------------------

FILE lcdTwiStream = FDEV_SETUP_STREAM(lcdTwiWriteStd, NULL, _FDEV_SETUP_WRITE);
lcdTwi_t *defaultDisplay = NULL;
...
void lcdTwiBacklight(lcdTwi_t *lcd, bool_t turnOn)
{
    ...
    setMaskOffset(message, (lcd->backlightOn), lcd->backlightBit);
...
void lcdTwiSetPort(lcdTwi_t *lcd, uint8_t controlE, uint8_t controlRs, uint8_t controlRw, uint8_t dataFirst,
...
void lcdTwiFunctionSet8Bits(lcdTwi_t *lcd, uint8_t command)
{
    uint8_t message[2] = {0, 0};

    // Backlight configuration
    setMaskOffset(message[0], (lcd->backlightOn), lcd->backlightBit);
    setMaskOffset(message[1], (lcd->backlightOn), lcd->backlightBit);

    // Higher nibble
    setMaskOffset(message[0], (command >> 4), lcd->dataFirst);	// Writes data
    setBit(message[0], lcd->controlE);						// Enable pulse
    setMaskOffset(message[1], (command >> 4), lcd->dataFirst);	// Writes data
...
void lcdTwiSendData(lcdTwi_t *lcd, uint8_t data, lcdTwiDataType_t type)
{
    ...
    setMaskOffset(message[0], (lcd->backlightOn), lcd->backlightBit);
    setMaskOffset(message[1], (lcd->backlightOn), lcd->backlightBit);
    setMaskOffset(message[2], (lcd->backlightOn), lcd->backlightBit);
    setMaskOffset(message[3], (lcd->backlightOn), lcd->backlightBit);

    // Higher nibble
    {
        setMaskOffset(message[0], (data >> 4), lcd->dataFirst);    // Writes data
    }
    setBit(message[0], lcd->controlE);					// Enable pulse
    setMaskOffset(message[1], (data >> 4), lcd->dataFirst);	// Writes data
    // Lower nibble
    setMaskOffset(message[2], (data & 0x0F), lcd->dataFirst);	// Writes data
    setBit(message[2], lcd->controlE);					// Enable pulse
    setMaskOffset(message[3], (data & 0x0F), lcd->dataFirst);	// Writes data
...
int16_t lcdTwiWriteStd(int8_t character, FILE *stream)
{

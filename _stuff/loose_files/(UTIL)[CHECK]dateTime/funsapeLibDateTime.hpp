 ...
 * Module:			Date and Time definitions for FunSAPE++ Embedded Library project
...
#	error	[funsapeLibDateTime.hpp] Error 12 - Global definitions file (funsapeLibGlobalDefines.hpp) is missing or corrupted!
#elif __FUNSAPE_LIB_GLOBAL_DEFINES_HPP != __BUILD_TO_REPLACE__
#	error	[funsapeLibDateTime.hpp] Error 13 - Build mismatch between global definitions file (funsapeLibGlobalDefines.hpp) and current header file (funsapeLibDateTime.hpp)!
#endif
...
#	error	[funsapeLibDateTime.hpp] Error 14 - Required module (funsapeLibDebug.hpp) is missing or corrupted!
#elif __FUNSAPE_LIB_DEBUG_HPP != __BUILD_TO_REPLACE__
#	error	[funsapeLibDateTime.hpp] Error 15 - Build mismatch between required module (funsapeLibDebug.hpp) and current header file (funsapeLibDateTime.hpp)!
#endif
...
private:
	...
	Month			_month;						// Month number; valid values in range [1 to 12], 0 is reserved for NOT DEFINED
	uint16_t		_year			: 14;		// Year in four digits format valid values in range [1 to 9999], 0 is reserved for NOT DEFINED
	WeekDay			_weekDay;					// Week day number; valid values in range [1 to 7], 0 is reserved for NOT DEFINED

	//     /////////////////     TIME DATA MEMBERS     //////////////////     //
	AmPmFlag		_amPmFlag;					// Time flag if AM/PM mode is set
	uint8_t			_hours			: 5;		// Hour in 24-hours format; valid values in range [0 to 24], 25 is reserved for NOT DEFINED
...
inlined bool operator >(DateTime::Month a, int b)
{

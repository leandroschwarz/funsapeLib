...
#	error	[usart0.cpp] Error 100 - System configuration file (globalConfigs.hpp) is missing
#else
#	if __GLOBAL_CONFIGS_HPP != 1001
#		error 	[usart0.cpp] Error 101 - Build mismatch (globalConfigs.hpp and usart0.hpp)
...
#		error 	[usart0.cpp] Error 102 - Build mismatch (globalDefines.hpp and usart0.hpp)
#	endif		// __GLOBAL_CONFIGS_HPP != 1001
#endif		// __USE_OWN_DEFINITIONS_FILE == 1

#include "usart0.hpp"
#if __USART0_HPP != 1001
#	error [usart0.cpp] Error 103 - Build mismatch on header and source code files.
#endif
...
Usart0::Usart0(usartBaudRate_e baudRate, usartMode_e mode, usartFrameFormat_e frameFormat)
{
	...
	case USART_FRAME_FORMAT_7_E_2:
		break;
	default:
		systemConfiguration.announceError(ERROR_USART_UNSUPPORTED_CONFIGURATION_FRAME_FORMAT);
		return;
	}
	// Error checking - mode
	switch(mode) {
	...
	case USART_MODE_SYNCHRONOUS_SPI:
		break;
	default:
		systemConfiguration.announceError(ERROR_USART_UNSUPPORTED_CONFIGURATION_PARITY);
		return;
	}
	...
	case USART_BAUD_256000:
		break;
	default:
		systemConfiguration.announceError(ERROR_USART_UNSUPPORTED_CONFIGURATION_BAUD_RATE);
		return;
	}
	...
	systemConfiguration.announceError(ERROR_NONE);
}

bool_t Usart0::setParity(usartParity_e parity)
{
	// Error checking
	switch(parity) {
...
bool_t Usart0::setMode(usartMode_e mode)
{
	// Error checking
	switch(mode) {
	case USART_MODE_ASYNCHRONOUS:
...
bool_t Usart0::setBaudRate(usartBaudRate_e baudRate)
{
	// Error checking
	switch(baudRate) {
	case USART_BAUD_600:
...
bool_t Usart0::setStopBit(usartStopBit_e stopBit)
{
	// Error checking
	switch(stopBit) {
	...
	case USART_STOP_BIT_DOUBLE:
		break;
	default:
		systemConfiguration.announceError(ERROR_USART_UNSUPPORTED_CONFIGURATION_STOP_BIT);
		return FALSE;
	}
...
bool_t Usart0::setDataBits(usartDataBits_e dataBits)
{
	// Error checking
	switch(dataBits) {
	...
	case USART_DATA_BITS_9:
		break;
	default:
		systemConfiguration.announceError(ERROR_USART_UNSUPPORTED_CONFIGURATION_DATA_BITS);
		return FALSE;
	}
...
bool_t Usart0::initialize(void)
{
	...
	if(this->configFlags_ != USART_CONFIG_FLAGS_ALL) {
		systemConfiguration.announceError(ERROR_USART_CONFIGURATION_PARAMETER_WRONG);
		return FALSE;
	}
	...
	switch(this->mode_) {
	case USART_MODE_ASYNCHRONOUS:
		aux64 = (systemConfiguration.getCpuClock() / 16 / this->baudRate_) - 1;
		break;
	case USART_MODE_ASYNCHRONOUS_DOUBLE_SPEED:
		aux64 = (systemConfiguration.getCpuClock() / 8 / this->baudRate_) - 1;
		break;
	case USART_MODE_SYNCHRONOUS_SPI:
		aux64 = (systemConfiguration.getCpuClock() / 2 / this->baudRate_) - 1;
		break;
	}

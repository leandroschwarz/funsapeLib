...
#include "funsapeLibUart.hpp"
#ifndef __FUNSAPE_LIB_UART_HPP
#	error	[funsapeLibUart.cpp] Error 1 - Header file (funsapeLibUart.hpp) must be corrupted!
#elif __FUNSAPE_LIB_UART_HPP != __BUILD_TO_REPLACE__
#	error	[funsapeLibUart.cpp] Error 9 - Build mismatch between source (funsapeLibUart.cpp) and header (funsapeLibUart.hpp) files!
#endif
...
#define		UART_DEFAULT_TIME_OUT				30
#define		UART_MAX_BUFFER_SIZE				200
#define		UART_MIN_BUFFER_SIZE				5
...
bool Uart::init(UART_HandleTypeDef *handler, uint16_t bufSize)
{
	...
	if (bufSize == 0) {
		...
		return false;
	} else if (bufSize < UART_MIN_BUFFER_SIZE) {
		this->_lastError = ERROR_BUFFER_SIZE_TOO_SMALL;
		return false;
	} else if (bufSize > UART_MAX_BUFFER_SIZE) {
...
bool Uart::init(UART_HandleTypeDef *handler, uint16_t rxBufSize, uint16_t txBufSize)
{
	// Deinitialize peripheral
	if (this->_initialized) {
		this->_deinitialize();
	...
	if ((rxBufSize == 0) || (txBufSize == 0)) {
		this->_lastError = ERROR_BUFFER_SIZE_ZERO;
		return false;
	} else if ((rxBufSize < UART_MIN_BUFFER_SIZE) || (txBufSize < UART_MIN_BUFFER_SIZE)) {
		this->_lastError = ERROR_BUFFER_SIZE_TOO_SMALL;
		return false;
	} else if ((rxBufSize > UART_MAX_BUFFER_SIZE) || (txBufSize > UART_MAX_BUFFER_SIZE)) {
		this->_lastError = ERROR_BUFFER_SIZE_TOO_LARGE;
		return false;
	}
...
// bool Uart::init(uint16_t bufSize = 20)
// {
// 	// Deinitialize peripheral
// 	if (this->_initialized) {
// 		this->_deinitialize();
...
// bool Uart::init(uint16_t rxBufSize, uint16_t txBufSize)
// {
// 	// Deinitialize peripheral
// 	if (this->_initialized) {
// 		this->_deinitialize();
...
// bool Uart::setTrasnmissionMode(spiTransmissionMode_t mode)
// {
// 	// Update data members
...
bool Uart::setTimeOut(uint16_t timeOut)
{
	// Update data members
...
bool Uart::setDefaultDevice(void actFuncCall(void), void deactFuncCall(void))
{
	// Check for errors
...
bool Uart::sendData(uint8_t *msg, uint16_t msgSize)
{
	// Check for errors
	if (!this->_initialized) {
...
bool Uart::sendData(uint8_t reg, uint8_t *msg, uint16_t msgSize)
{
	// Check for errors
	if (!this->_initialized) {
		this->_lastError = ERROR_NOT_INITIALIZED;
...
error_e Uart::getLastError(void)
{
	// Return error
...
#ifdef _FUNSAPE_PLATFORM_ARM_STM32
	HAL_UART_StateTypeDef spiState = HAL_UART_STATE_RESET;

	spiState = HAL_UART_GetState(this->_spiHandler);
	if (spiState != HAL_UART_STATE_READY) {
		// Return error
		this->_initialized = false;
...
//	HAL_UART_Transmit(&huart1, (uint8_t *)teste, strlen(teste), 30);
	...
	halStatus = HAL_UART_TransmitReceive(this->_spiHandler, this->_txBufferData, this->_rxBufferData, this->_txBufferLength,
			this->_timeOut);
	...
	if (halStatus == HAL_ERROR) {
		switch (halUartError) {
			case HAL_UART_ERROR_FLAG:
				this->_lastError = ERROR_UART_BUSY_FLAG;
				break;
			case HAL_UART_ERROR_CRC:
				this->_lastError = ERROR_UART_CRC;
				break;
			case HAL_UART_ERROR_OVR:
				this->_lastError = ERROR_UART_OVERRUN;
				break;
			case HAL_UART_ERROR_MODF:
				this->_lastError = ERROR_UART_MODE_FAULT;
				break;
			case HAL_UART_ERROR_DMA:
				this->_lastError = ERROR_UART_DMA;
				break;
			default:

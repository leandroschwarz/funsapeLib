...
bool Twi::_transmit(void)
{
	...
	if(this->_operation == TwiOperation::READ) {
		twiStatus = HAL_I2C_Mem_Read(this->_busHandler, (this->_devAddress << 1), this->_regAddress, I2C_MEMADD_SIZE_8BIT, this->_bufferData, this->_bufferLength, this->_timeout);
	} else if(this->_operation == TwiOperation::WRITE) {
		twiStatus = HAL_I2C_Mem_Write(this->_busHandler, (this->_devAddress << 1), this->_regAddress, I2C_MEMADD_SIZE_8BIT, this->_bufferData, this->_bufferLength, this->_timeout);
	} else {
		// Returns error

...
void twiMasterInit(uint32_t clockSpeed)
{
	...
	aux32 = (uint32_t)systemConfiguration.cpuClock / (uint32_t)clockSpeed;

	if(aux32 <= 526) {					// Prescaler 1
		aux8 = (uint8_t)((((uint32_t)systemConfiguration.cpuClock / (uint32_t)clockSpeed) - 16) / 2);
		clrBit(TWSR, TWPS1);
		clrBit(TWSR, TWPS0);
	} else if(aux32 <= 2056) {			// Prescaler 4
		aux8 = (uint8_t)((((uint32_t)systemConfiguration.cpuClock / (uint32_t)clockSpeed) - 16) / 8);
		clrBit(TWSR, TWPS1);
		setBit(TWSR, TWPS0);
	} else if(aux32 <= 8176) {			// Prescaler 16
		aux8 = (uint8_t)((((uint32_t)systemConfiguration.cpuClock / (uint32_t)clockSpeed) - 16) / 32);
		setBit(TWSR, TWPS1);
		clrBit(TWSR, TWPS0);
	} else if(aux32 <= 32656) {			// Prescaler 64
		aux8 = (uint8_t)((((uint32_t)systemConfiguration.cpuClock / (uint32_t)clockSpeed) - 16) / 128);
		setBit(TWSR, TWPS1);
		setBit(TWSR, TWPS0);
	} else {
		return;

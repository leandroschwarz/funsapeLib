...
bool Twi::writeReg(uint8_t reg_p, uint8_t *buffData_p, uint16_t buffSize_p)
{
	...
	twiStatus = HAL_I2C_Mem_Write(this->_twiBus, (this->_devAddress << 1), this->_regAddress, I2C_MEMADD_SIZE_8BIT, this->_bufferData, this->_bufferLength, this->_timeout);
	...
	if(twiStatus == HAL_TIMEOUT) {
		...
		// Return error
		uint32_t halTwiError = HAL_I2C_GetError(this->_twiBus);
...
bool Twi::readReg(uint8_t reg_p, uint8_t *buffData_p, uint16_t buffSize_p)
{
	...
	// Reading from bus
	this->_busy = true;							// Mark peripheral as busy
	twiStatus = HAL_I2C_Mem_Read(this->_twiBus, (this->_devAddress << 1), this->_regAddress, I2C_MEMADD_SIZE_8BIT, this->_bufferData, this->_bufferLength, this->_timeout);
...
bool Twi::_initialize(void)
{
	...
	do {
		...
		twiState = HAL_I2C_GetState(this->_twiBus);
		switch(twiState) {

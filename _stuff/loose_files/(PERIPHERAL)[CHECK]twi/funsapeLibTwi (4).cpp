...
bool Twi0::setMaster(uint32_t bitRate)
{
	...
	clrMask(TWI0_CLOCK_PRESCALER_REG, 0x03, TWI0_CLOCK_PRESCALER_BIT);
	setMask(TWI0_CLOCK_PRESCALER_REG, this->_bitRatePrescaler, TWI0_CLOCK_PRESCALER_BIT);
...
void Twi0::interruptHandler(void)
{
	...
	case TWI_STX_DATA_ACK:
		TWI0_DATA_REG = twiBuffer[twiIndex];
		twiIndex++;
		if (twiIndex == TWI_BUFFER_SIZE) {
			twiIndex = 0;
		}
	...
	case TWI_SRX_GEN_DATA_ACK:
		if (twiAddrByte) {
			twiAddrByte = 0;
			twiIndex = TWI0_DATA_REG;
			if (twiIndex >= TWI_BUFFER_SIZE) {
				twiIndex = 0;
			}
		} else {
			twiBuffer[twiIndex] = TWI0_DATA_REG;
			twiIndex++;
			if (twiIndex == TWI_BUFFER_SIZE) {
				twiIndex = 0;
			}
	...
	case TWI_STX_DATA_ACK_LAST_BYTE:
		TWI0_CONTROL_REG =	(1 << TWI0_INT_FLAG_BIT) |
							(1 << TWI0_SEND_STOP_BIT);
		break;

	case TWI_BUS_ERROR:
		if (this->_slaveEnabled) {
			TWI0_CONTROL_REG =	(1 << TWI0_INT_FLAG_BIT) |
								(1 << TWI0_SEND_STOP_BIT);
		} else if (this->_masterEnabled) {
			TWI0_CONTROL_REG =	(1 << TWI0_ENABLE_BIT) |
								(1 << TWI0_SEND_STOP_BIT);
		}
		this->_state = (twiState_e)TWI0_STATUS_REG;		// Store TWI0_STATUS_REG and automatically sets clears noErrors bit
...
bool Twi0::setSlave(uint8_t myAddress, bool generalCallsEnabled, uint8_t addressMask)
{
	// Check for errors (slave address)
	if (myAddress > 127) {								// Invalid slave address
		this->_lastError = ERROR_SLAVE_ADDRESS_INVALID;		// Report error
		return false;										// Return with error
	} else if ((myAddress & 0x78) == 0x00) {			// Reserved slave address
		this->_lastError = ERROR_SLAVE_ADDRESS_RESERVED;	// Report error
		return false;										// Return with error
	} else if ((myAddress & 0x78) == 0x78) {			// Reserved slave address
		this->_lastError = ERROR_SLAVE_ADDRESS_RESERVED;	// Report error
		return false;										// Return with error
	}
	// Check for errors (mask address)
	if (addressMask > 127) {								// Invalid slave address
		this->_lastError = ERROR_SLAVE_ADDRESS_MASK_INVALID;	// Report error
		return false;											// Return with error
	}

	TWI0_SLAVE_ADDRESS_REG =	(myAddress << TWI0_SLAVE_ADDRESS_BIT) |
								(generalCallsEnabled << TWI0_ANSWER_GENERAL_CALL_BIT);

	TWI0_SLAVE_MASK_REG =		(addressMask << TWI0_SLAVE_ADDRESS_BIT);

	TWI0_CONTROL_REG =		(1 << TWI0_ENABLE_BIT) |
							(1 << TWI0_INT_ENABLE_BIT) |
							(1 << TWI0_INT_FLAG_BIT) |
							(1 << TWI0_SEND_ACK_BIT);
	this->_slaveEnabled = true;
...
uint8_t * Twi0::twiGetBuffer(void)
{
	return twiBuffer;
}

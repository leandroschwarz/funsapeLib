...
#define SPI_CLK_FAST  SPCR = ((SPCR/* | (1<<SPI2X)*/)  & ~((1<<SPR1) | (1<<SPR0))) /*F_CPU/2*/
#define SPI_CLK_LOW  SPCR |= (/*(1<<SPI2X) | */(1<<SPR1) | (1<<SPR0))			   /*F_CPU/64*/
...
{
	do {
		*buff++ = (BYTE) SPI_ReceiveByte();			/* Store a received byte */
	} while (--bc);
}

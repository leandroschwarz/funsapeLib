/*!
@file
@brief This file is marvelous ssadsadsa.
*/
/* -----------------------------------------------------------------------------
...
/*!	\struct systemConfiguration_s
	\brief System configuration variables

	Structure that holds some configurations values that are used throughout the library.
*/
typedef volatile struct systemConfiguration_s {
	uint16_t	systemClockKHz;		/*!< Holds the clock frequency (in kHz) of the system to be used throughout the library. Must be set calling systemClock(). Default values is 0. If not set at the beginning of the main() function, some functions may preventively halt the system. */
} systemConfiguration_s;

/*!	\struct	dateTime_s
	\brief	Data type to handle date and time in RTCs.

	More detailed description.
*/
typedef struct dateTime_s {
	uint16_t	year		: 7;		/*!< Holds the year, must be added to 2000 to be shown in four digits format. Valid values in range [0 to 99]. */
	month_t		month		: 4;		/*!< Holds the month number. Valid values in range [1 to 12], 0 is reserved for NOT DEFINED. */
	uint8_t		day			: 5;		/*!< Holds the month day. Valid values in range [1 to 31], 0 is reserved for NOT DEFINED. */
	weekDay_t	weekDay		: 3;		/*!< Holds the week day number. Valid values in range [1 to 7], 0 is reserved for NOT DEFINED. */
	uint8_t		hour		: 5;		/*!< Holds the hour in 24-hours format. Valid values in range [0 to 24]. */
	uint8_t		minute		: 6;		/*!< Holds the minutes. Valid values in range [0 to 59]. */
	uint8_t		second		: 6;		/*!< Holds the seconds. Valid values in range [0 to 59]. */
	//! @private

...
typedef enum error_e {
	...
	ERROR_SIZE_CANNOT_BE_ZERO,							// Buffer size was set to zero
...
public:
	DateTime();
	void getDate(uint8_t *day, month_t *month = NULL, uint16_t *year = NULL);
	bool_t setDate(uint8_t day, month_t month, uint16_t year);
	bool_t setTime(uint8_t hour, uint8_t minute, uint8_t second, utcTimeZone_e timeZone = UTC_MINUS_0300);
	bool_t setTime(uint8_t hour, uint8_t minute, uint8_t second, uint16_t millisecond = 0, utcTimeZone_e timeZone = UTC_MINUS_0300);

private:
	bool_t isLeapYear(uint16_t year);
	void evaluateDayOfWeek(void);

private:
	uint32_t		year_			: 11;		// Year in four digits format; valid values in range [0 to 2047]
	month_t			month_			: 4;		// Month number; valid values in range [1 to 12], 0 is reserved for NOT DEFINED
	uint32_t		day_			: 5;		// Month day number; valid values in range [1 to 31], 0 is reserved for NOT DEFINED
	weekDay_t		weekDay_		: 3;		// Week day number; valid values in range [1 to 7], 0 is reserved for NOT DEFINED
	uint32_t		hour_			: 5;		// Hour in 24-hours format; valid values in range [0 to 24]
	uint32_t		minute_			: 6;		// Minutes; valid values in range [0 to 59]
	uint32_t		second_			: 6;		// Seconds; valid values in range [0 to 59]
	uint32_t		millisecond_	: 10;		// Milliseconds; valid values in range [0 to 999]
	bool_t			leapYear_		: 1;		// If the year is a leap year
	bool_t			summerTime_		: 1;		// If summer time is active
	utcTimeZone_e	timeZone_		: 6;		// Current time zone
	uint32_t		unusedBits_		: 6;		// Unused bits for future expansion
};

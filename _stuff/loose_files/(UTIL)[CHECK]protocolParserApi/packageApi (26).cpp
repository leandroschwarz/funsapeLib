...
#if __GPDSE_ARM_PACKAGE_API_H != __BUILD_MACRO_TO_BE_CHANGED__
#	error Build mismatch on header and source code files (packageApi).
#endif
...
bool PackageApi::rxAddData(uint8_t data)
{
	...
	if (this->rxRawReady_) {
		this->lastError_ = ERROR_PACKAGE_AWAITING;
		return false;
	}
...
bool PackageApi::rxProcessEscapeCharacter(void)
{
	...
	if (this->rxDecodedAuxData_ == this->configEscapeCharacter_) {
		...
		if (this->rxRawEscape_) {
			...
			this->lastError_ = ERROR_FRAME_ERROR;
			this->rxResetPackage();
			return false;
		} else {
...
bool PackageApi::rxProcessFrameFormat(void)
{
	...
	case STATE_CHECKSUM:
		if (this->rxDecodedAuxData_ != this->rxRawChecksum_) {	// Checksum error
			this->rxResetPackage();
			this->lastError_ = ERROR_CHECKSUM_ERROR;
			return false;
		}
	default:
		break;
...
bool PackageApi::rxProcessNextState(void)
{
	...
	case STATE_START_DELIMITER:
		this->rxRawChecksum_ = 0xFF;
		this->rxRawState_ = (state_e)(this->rxRawState_ + 1);
		break;
	case STATE_ID:

...
bool Mpu::setAccelAxis(Axis axis_p)
{
	...
	auxBuff = 0;
	setMask(auxBuff, !(auxBoolX), Bit::PWR_MGMT_2_DISABLE_XA);
	setMask(auxBuff, !(auxBoolY), Bit::PWR_MGMT_2_DISABLE_YA);
	setMask(auxBuff, !(auxBoolZ), Bit::PWR_MGMT_2_DISABLE_ZA);
	setMask(auxBuff, !((bool)(this->_gyroAxisEnabled & Axis::X)), Bit::PWR_MGMT_2_DISABLE_XA);
	setMask(auxBuff, !((bool)(this->_gyroAxisEnabled & Axis::Y)), Bit::PWR_MGMT_2_DISABLE_YA);
	setMask(auxBuff, !((bool)(this->_gyroAxisEnabled & Axis::Z)), Bit::PWR_MGMT_2_DISABLE_ZA);
...
bool Mpu::setAccelDlpf(AccelDlpf dlpf_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, dlpfDisabled, Bit::ACCEL_CONFIG2_ACCEL_FCHOICE_B);
	setMask(auxBuff, (uint8_t)(dlpf_p), Bit::ACCEL_CONFIG2_A_DLPFCFG);
...
bool Mpu::setAccelFullScale(AccelFullScale fullScale_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, (uint8_t)(fullScale_p), Bit::ACCEL_CONFIG_ACCEL_FS_SEL);
...
bool Mpu::setAccelOffset(int16_t *buff_p, Axis axis_p)
{
	...
	if((axis_p & Axis::X) == Axis::X) {
		...
		setMask(auxBuff[1], (auxInt16 & BitMask::XA_OFFSET_L), Bit::XA_OFFSET_L);
		auxInt16 >>= 7;
		setMask(auxBuff[0], (auxInt16 & BitMask::XA_OFFSET_H), Bit::XA_OFFSET_H);
		// Sends data to device
	...
	if((axis_p & Axis::Y) == Axis::Y) {
		// Process transmission buffer
		auxInt16 = buff_p[index++];
		setMask(auxBuff[1], (auxInt16 & BitMask::YA_OFFSET_L), Bit::YA_OFFSET_L);
		auxInt16 >>= 7;
		setMask(auxBuff[0], (auxInt16 & BitMask::YA_OFFSET_H), Bit::YA_OFFSET_H);
		// Sends data to device
	...
	if((axis_p & Axis::Z) == Axis::Z) {
		// Process transmission buffer
		auxInt16 = buff_p[index++];
		setMask(auxBuff[1], (auxInt16 & BitMask::ZA_OFFSET_L), Bit::ZA_OFFSET_L);
		auxInt16 >>= 7;
		setMask(auxBuff[0], (auxInt16 & BitMask::ZA_OFFSET_H), Bit::ZA_OFFSET_H);
		// Sends data to device
...
bool Mpu::setGyroAxis(Axis axis_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, !((bool)(this->_accelAxisEnabled & Axis::X)), Bit::PWR_MGMT_2_DISABLE_XA);
	setMask(auxBuff, !((bool)(this->_accelAxisEnabled & Axis::Y)), Bit::PWR_MGMT_2_DISABLE_YA);
	setMask(auxBuff, !((bool)(this->_accelAxisEnabled & Axis::Z)), Bit::PWR_MGMT_2_DISABLE_ZA);
	setMask(auxBuff, !(auxBoolX), Bit::PWR_MGMT_2_DISABLE_XG);
	setMask(auxBuff, !(auxBoolY), Bit::PWR_MGMT_2_DISABLE_YG);
	setMask(auxBuff, !(auxBoolZ), Bit::PWR_MGMT_2_DISABLE_ZG);
...
bool Mpu::setGyroDlpf(GyroDlpf dlpf_p)
{
	...
	auxBuff[0] = 0;
	auxBuff[1] = 0;
	setMask(auxBuff[0], this->_fifoRollOverDisabled, Bit::CONFIG_FIFO_MODE);
	setMask(auxBuff[0], (uint8_t)(this->_externalSync), Bit::CONFIG_EXT_SYNC_SET);
	setMask(auxBuff[0], (uint8_t)(dlpf_p), Bit::CONFIG_DLPF_CFG);
	setMask(auxBuff[1], fChoiceB, Bit::GYRO_CONFIG_FCHOICE_B);
	setMask(auxBuff[1], (uint8_t)(this->_gyroFullScale), Bit::GYRO_CONFIG_GYRO_FS_SEL);
...
bool Mpu::setGyroFullScale(GyroFullScale fullScale_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, (uint8_t)(fullScale_p), Bit::GYRO_CONFIG_GYRO_FS_SEL);
	setMask(auxBuff, this->_gyroFchoiceB, Bit::GYRO_CONFIG_FCHOICE_B);
...
bool Mpu::setGyroOffset(int16_t *buff_p, Axis axis_p)
{
	...
	if((axis_p & Axis::X) == Axis::X) {
		// Process transmission buffer
		auxInt16 = buff_p[index++];
		setMask(auxBuff[1], (auxInt16 & BitMask::XG_OFFSET_L), Bit::XG_OFFSET_L);
		auxInt16 >>= 8;
		setMask(auxBuff[0], (auxInt16 & BitMask::XG_OFFSET_H), Bit::XG_OFFSET_H);
		// Sends data to device
	...
	if((axis_p & Axis::Y) == Axis::Y) {
		// Process transmission buffer
		auxInt16 = buff_p[index++];
		setMask(auxBuff[1], (auxInt16 & BitMask::YG_OFFSET_L), Bit::YG_OFFSET_L);
		auxInt16 >>= 8;
		setMask(auxBuff[0], (auxInt16 & BitMask::YG_OFFSET_H), Bit::YG_OFFSET_H);
		// Sends data to device
	...
	if((axis_p & Axis::Z) == Axis::Z) {
		// Process transmission buffer
		auxInt16 = buff_p[index++];
		setMask(auxBuff[1], (auxInt16 & BitMask::ZG_OFFSET_L), Bit::ZG_OFFSET_L);
		auxInt16 >>= 8;
		setMask(auxBuff[0], (auxInt16 & BitMask::ZG_OFFSET_H), Bit::ZG_OFFSET_H);
		// Sends data to device
...
bool Mpu::configIntPin(LogicLevel pinLevel_p, DriverMode pinDriver_p, bool pinLatch_p, bool pinAutoReset_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, !(bool)(pinLevel_p), Bit::INT_PIN_CFG_ACTL);
	setMask(auxBuff, (bool)(pinDriver_p), Bit::INT_PIN_CFG_OPEN);
	setMask(auxBuff, (bool)(pinLatch_p), Bit::INT_PIN_CFG_LATCH_INT_EN);
	setMask(auxBuff, (bool)(pinAutoReset_p), Bit::INT_PIN_CFG_INT_ANYRD_2CLEAR);
	setMask(auxBuff, !(bool)(this->_fsyncIntPinActiveLevel), Bit::INT_PIN_CFG_ACTL_FSYNC);
	setMask(auxBuff, this->_fsyncPinTriggersInterrupt, Bit::INT_PIN_CFG_FSYNC_INT_MODE_EN);
	setMask(auxBuff, this->_i2cRoutedToAuxBus, Bit::INT_PIN_CFG_BYPASS_EN);
...
bool Mpu::disableInterrupts(InterruptFlags deactivationMask_p)
{
	...
	// Process transmission buffer
	auxBuff = 0;
	setMask(auxBuff, intWakeUpOnMotion, Bit::INT_ENABLE_WOM_EN);
	setMask(auxBuff, intFifoOverflow, Bit::INT_ENABLE_FIFO_OVERFLOW_EN);
	setMask(auxBuff, intFsyncPin, Bit::INT_ENABLE_FSYNC_INT_EN);
	setMask(auxBuff, intRawDataReady, Bit::INT_ENABLE_RAW_RDY_EN);

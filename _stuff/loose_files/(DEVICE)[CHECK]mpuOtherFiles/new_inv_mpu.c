...
int NEW_Sensors_I2C_WriteRegister(unsigned char slave_addr, unsigned char reg_addr, unsigned short len, const unsigned char *data_ptr)
{
	uint16_t Size = 0;
	uint8_t buffer[100];

	buffer[Size++] = reg_addr;
	slave_addr <<= 1;
	for(uint8_t i = 0; i < len; i++) {
		buffer[Size++] = data_ptr[i];
	}
...
int NEW_Sensors_I2C_ReadRegister(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data)
{
	data[0] = reg_addr;
	slave_addr <<= 1;
...
#define i2c_write   NEW_Sensors_I2C_WriteRegister
// #define i2c_read    Sensors_I2C_ReadRegister
#define i2c_read    NEW_Sensors_I2C_ReadRegister
//////////////////////////////////////// DEBUG END
#define delay_ms    HAL_Delay
#define get_ms      get_tick_count
#define log_i       MPL_LOGI
//////////////////////////////////////// DEBUG START
#define log_e       MY_MPL_LOGE
 ...
 *  @return     0 if successful.
 */
int new_mpu_read_reg(unsigned char reg, unsigned char *data)
{
...
int new_mpu_init(struct int_param_s *int_param)
{
...
#endif
	/* new_mpu_set_sensors always preserves this setting. */
	st.chip_cfg.clk_src = INV_CLK_PLL;
	/* Handled in next call to new_mpu_set_bypass. */
	...
	if(new_mpu_set_gyro_fsr(2000)) {
		return -1;
	}
	if(new_mpu_set_accel_fsr(2)) {
		return -1;
	}
	if(new_mpu_set_lpf(42)) {
		return -1;
	}
	if(new_mpu_set_sample_rate(50)) {
		return -1;
	}
	if(new_mpu_configure_fifo(0)) {
		return -1;
	}

#ifdef AK89xx_SECONDARY
	setup_compass();
	if(new_mpu_set_compass_sample_rate(10)) {
		return -1;
	}
#else
	/* Already disabled by setup_compass. */
	if(new_mpu_set_bypass(0)) {
		return -1;
	}
#endif

	new_mpu_set_sensors(0);
	return 0;
}
 ...
 *  @return     0 if successful.
 */
int new_mpu_lp_accel_mode(unsigned short rate)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
	unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_accel_reg(short *data, unsigned long *timestamp)
{
	unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_temperature(long *data, unsigned long *timestamp)
{
	unsigned char tmp[2];
	short raw;
 ...
 *  @return     0 if successful.
 */
int new_mpu_read_6500_accel_bias(long *accel_bias)
{
	unsigned char data[6];
 ...
 *  @return     0 if successful.
 */
int new_mpu_read_6050_accel_bias(long *accel_bias)
{
	unsigned char data[6];
...
int new_mpu_read_6500_gyro_bias(long *gyro_bias)
{
	unsigned char data[6];
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_gyro_bias_reg(long *gyro_bias)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_accel_bias_6050_reg(const long *accel_bias)
{
	unsigned char data[6] = {0, 0, 0, 0, 0, 0};
	long accel_reg_bias[3] = {0, 0, 0};

	if(new_mpu_read_6050_accel_bias(accel_reg_bias)) {
		return -1;
	}
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_accel_bias_6500_reg(const long *accel_bias)
{
	unsigned char data[6] = {0, 0, 0, 0, 0, 0};
	long accel_reg_bias[3] = {0, 0, 0};

	if(new_mpu_read_6500_accel_bias(accel_reg_bias)) {
		return -1;
	}
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_gyro_fsr(unsigned short *fsr)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_gyro_fsr(unsigned short fsr)
{
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_accel_fsr(unsigned char *fsr)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_accel_fsr(unsigned char fsr)
{
	unsigned char data;
 ...
 *  0 if successful.
 */
int new_mpu_get_lpf(unsigned short *lpf)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_lpf(unsigned short lpf)
{
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_sample_rate(unsigned short *rate)
{
	if(st.chip_cfg.dmp_on) {
		return -1;
	} else {
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_sample_rate(unsigned short rate)
{
	unsigned char data;
	...
	if(st.chip_cfg.dmp_on) {
		...
		if(st.chip_cfg.lp_accel_mode) {
			if(rate && (rate <= 40)) {
				...
				new_mpu_lp_accel_mode(rate);
				return 0;
			}
			/* Requested rate exceeds the allowed frequencies in LP accel mode,
			 * switch back to full-power mode.
			 */
			new_mpu_lp_accel_mode(0);
		}
		if(rate < 4) {
			rate = 4;
		} else if(rate > 1000) {
...
#ifdef AK89xx_SECONDARY
		new_mpu_set_compass_sample_rate(min(st.chip_cfg.compass_sample_rate, MAX_COMPASS_SAMPLE_RATE));
#endif

		/* Automatically set LPF to 1/2 sampling rate. */
		new_mpu_set_lpf(st.chip_cfg.sample_rate >> 1);
		return 0;
	}
}
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_compass_sample_rate(unsigned short *rate)
{
#ifdef AK89xx_SECONDARY
 ...
 *  new_mpu_get_compass_sample_rate to check the actual setting.
 *  @param[in]  rate    Desired compass sampling rate (Hz).
 *  @return     0 if successful.
 */
int new_mpu_set_compass_sample_rate(unsigned short rate)
{
#ifdef AK89xx_SECONDARY
	unsigned char div;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_gyro_sens(float *sens)
{
	switch(st.chip_cfg.gyro_fsr) {
	case INV_FSR_250DPS:
		sens[0] = 131.f;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_accel_sens(unsigned short *sens)
{
	switch(st.chip_cfg.accel_fsr) {
	case INV_FSR_2G:
		sens[0] = 16384;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_fifo_config(unsigned char *sensors)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_configure_fifo(unsigned char sensors)
{
	unsigned char prev;
	int result = 0;
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_power_state(unsigned char *power_on)
{
	if(st.chip_cfg.sensors) {
		power_on[0] = 1;
	} else {
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_sensors(unsigned char sensors)
{
	unsigned char data;
#ifdef AK89xx_SECONDARY
	unsigned char user_ctrl;
...
#	ifdef AK89xx_BYPASS
	if(sensors & INV_XYZ_COMPASS) {
		new_mpu_set_bypass(1);
	} else {
		new_mpu_set_bypass(0);
	}
#	else
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_int_status(short *status)
{
	unsigned char tmp[2];
	if(!st.chip_cfg.sensors) {
		return -1;
 ...
 *  @return     0 if successful.
 */
int new_mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp, unsigned char *sensors, unsigned char *more)
{
 ...
 */
int new_mpu_read_fifo_stream(unsigned short length, unsigned char *data,
		unsigned char *more)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_bypass(unsigned char bypass_on)
{
	unsigned char tmp;
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_int_level(unsigned char active_low)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_int_latched(unsigned char enable)
{
	unsigned char tmp;
...
static const unsigned short new_mpu_6500_st_tb[256] = {
...
static int accel_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	for(i = 0; i < 3; i++) {
		if(regs[i] != 0) {
			ct_shift_prod[i] = new_mpu_6500_st_tb[regs[i] - 1];
 ...
 */
int new_mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
...
#	endif
	...
	new_mpu_get_gyro_fsr(&gyro_fsr);
	new_mpu_get_accel_fsr(&accel_fsr);
	new_mpu_get_lpf(&lpf);
	new_mpu_get_sample_rate(&sample_rate);
	sensors_on = st.chip_cfg.sensors;
	new_mpu_get_fifo_config(&fifo_sensors);
...
restore:
	...
	st.chip_cfg.fifo_enable = 0xFF;
	st.chip_cfg.clk_src = INV_CLK_PLL;
	new_mpu_set_gyro_fsr(gyro_fsr);
	new_mpu_set_accel_fsr(accel_fsr);
	new_mpu_set_lpf(lpf);
	new_mpu_set_sample_rate(sample_rate);
	new_mpu_set_sensors(sensors_on);
	new_mpu_configure_fifo(fifo_sensors);

	if(dmp_was_on) {
		new_mpu_set_dmp_state(1);
	}
...
*  @return     Result mask (see above).
*/
int new_mpu_run_self_test(long *gyro, long *accel)
{
#ifdef MPU6050
	const unsigned char tries = 2;
	long gyro_st[3], accel_st[3];
 ...
 *  @return     0 if successful.
 */
int new_mpu_write_mem(unsigned short mem_addr, unsigned short length, unsigned char *data)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int new_mpu_read_mem(unsigned short mem_addr, unsigned short length, unsigned char *data)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int new_mpu_load_firmware(unsigned short length, const unsigned char *firmware, unsigned short start_addr, unsigned short sample_rate)
{
	unsigned short ii;
...
#define LOAD_CHUNK  (16)
	...
	for(ii = 0; ii < length; ii += this_write) {
		...
		if(new_mpu_write_mem(ii, this_write, (unsigned char *)&firmware[ii])) {
			return -1;
		}
		if(new_mpu_read_mem(ii, this_write, cur)) {
			return -1;
		}
 ...
 *  @return     0 if successful.
 */
int new_mpu_set_dmp_state(unsigned char enable)
{
	unsigned char tmp;
	...
	if(enable) {
		...
		new_mpu_set_sample_rate(st.chip_cfg.dmp_sample_rate);
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_dmp_state(unsigned char *enabled)
{
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
#ifdef AK89xx_SECONDARY
	unsigned char tmp[9];
 ...
 *  @return     0 if successful.
 */
int new_mpu_get_compass_fsr(unsigned short *fsr)
{
#ifdef AK89xx_SECONDARY
 ...
 *  @return     0 if successful.
 */
int new_mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time,
		unsigned short lpa_freq)
{
	...
	if(lpa_freq) {
		...
		if(!st.chip_cfg.int_motion_only) {
			...
			new_mpu_get_gyro_fsr(&st.chip_cfg.cache.gyro_fsr);
			new_mpu_get_accel_fsr(&st.chip_cfg.cache.accel_fsr);
			new_mpu_get_lpf(&st.chip_cfg.cache.lpf);
			new_mpu_get_sample_rate(&st.chip_cfg.cache.sample_rate);
			st.chip_cfg.cache.sensors_on = st.chip_cfg.sensors;
			new_mpu_get_fifo_config(&st.chip_cfg.cache.fifo_sensors);
		}
...
lp_int_restore:
	...
	st.chip_cfg.fifo_enable = 0xFF;
	st.chip_cfg.clk_src = INV_CLK_PLL;
	new_mpu_set_sensors(st.chip_cfg.cache.sensors_on);
	new_mpu_set_gyro_fsr(st.chip_cfg.cache.gyro_fsr);
	new_mpu_set_accel_fsr(st.chip_cfg.cache.accel_fsr);
	new_mpu_set_lpf(st.chip_cfg.cache.lpf);
	new_mpu_set_sample_rate(st.chip_cfg.cache.sample_rate);
	new_mpu_configure_fifo(st.chip_cfg.cache.fifo_sensors);

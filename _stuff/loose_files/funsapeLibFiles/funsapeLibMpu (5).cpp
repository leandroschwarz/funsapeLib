...
#elif __FUNSAPE_LIB_MPU_HPP != __BUILD_NUMBER__
...
Mpu::Mpu(PartId partId_p)
{
    ...
    debugMark("Mpu::Mpu(PartId)", Debug::CodeIndex::MPU_MODULE);
...
{
    // Mark passage for debugging purpose
    debugMark("Mpu::~Mpu(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::configAccel(Axis axis_p, AccelFullScale fullScale_p, AccelDlpf dlpf_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::configAccel(Axis, AccelFullScale, AccelDlpf)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setAccelAxis(Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setAccelAxis(Axis)", Debug::CodeIndex::MPU_MODULE);

    // Local variables
    uint8_t auxBuff = 0;
    bool_t auxBoolX = false;
    bool_t auxBoolY = false;
    bool_t auxBoolZ = false;
...
bool_t Mpu::setAccelDlpf(AccelDlpf dlpf_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setAccelDlpf(AccelDlpf)", Debug::CodeIndex::MPU_MODULE);

    // Local variables
    uint8_t auxBuff = 0;
    bool_t dlpfDisabled = false;
...
bool_t Mpu::setAccelFullScale(AccelFullScale fullScale_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setAccelFullScale(AccelFullScale)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setAccelLowPowerMode(uint8_t sampleRate_p)
{
    // FIXME: Implement -> bool_t Mpu::setAccelLowPowerMode(uint8_t sampleRate_p)
    // Mark passage for debugging purpose
    debugMark("Mpu::setAccelLowPowerMode(uint8_t)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::getAccelData(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::getAccelData(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
    ...
    if (axis_p == Axis::NONE) {
        ...
        debugMessage(Error::AXIS_INVALID);
        return false;
    }
...
bool_t Mpu::getAccelOffset(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::getAccelOffset(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setAccelOffset(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setAccelOffset(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setCompassSampleRate(uint8_t sampleRate_p)
{
    // FIXME: Implement -> bool_t Mpu::setCompassSampleRate(uint8_t sampleRate_p)
    // Mark passage for debugging purpose
    debugMark("Mpu::setCompassSampleRate(uint8_t)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setupCompass(void)
{
    // FIXME: Implement -> bool_t Mpu::setupCompass(void)
    // Mark passage for debugging purpose
    debugMark("Mpu::setupCompass(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::configFifo(uint8_t sensors_p)
{
    // FIXME: Implement -> bool_t Mpu::configFifo(uint8_t sensors_p)
    // Mark passage for debugging purpose
    debugMark("Mpu::configFifo(uint8_t)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::configGyro(Axis axis_p, GyroFullScale fullScale_p, GyroDlpf dlpf_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::configGyro(Axis, GyroFullScale, GyroDlpf)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setGyroAxis(Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setGyroAxis(Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setGyroDlpf(GyroDlpf dlpf_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setGyroDlpf(GyroDlpf)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setGyroFullScale(GyroFullScale fullScale_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setGyroFullScale(GyroFullScale)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::getGyroData(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::getGyroData(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::getGyroOffset(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::getGyroOffset(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setGyroOffset(int16_t *buff_p, Axis axis_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setGyroOffset(int16_t *, Axis)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::configIntPin(LogicLevel pinLevel_p, DriverMode pinDriver_p, bool_t pinLatch_p, bool_t pinAutoReset_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::configIntPin(LogicLevel, DriverMode, bool, bool)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::disableInterrupts(InterruptFlags deactivationMask_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::disableInterrupts(InterruptFlags)", Debug::CodeIndex::MPU_MODULE);

    // Local variables
    uint8_t auxBuff;
    bool_t intFifoOverflow                = false;
    bool_t intFsyncPin                    = false;
    bool_t intRawDataReady                = false;
    bool_t intWakeUpOnMotion              = false;
...
bool_t Mpu::enableInterrupts(InterruptFlags activationMask_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::enableInterrupts(InterruptFlags)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::generalIrqHandler(void)
{
...
bool_t Mpu::setIntFifoOverflowCallback(void (*callbackFunction_p)(void))
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setIntFifoOverflowCallback(void (*)(void))", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setIntFsyncPinCallback(void (*callbackFunction_p)(void))
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setIntFsyncPinCallback(void (*)(void))", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setIntRawDataReadyCallback(void (*callbackFunction_p)(void))
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setIntRawDataReadyCallback(void (*)(void))", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setIntWakeUpOnMotionCallback(void (*callbackFunction_p)(void))
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setIntWakeUpOnMotionCallback(void (*)(void))", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::init(Bus *twiBusHandler_p, bool_t useTwiAlternateAddress_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::init(Bus *, bool)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::init(Bus *spiBusHandler_p, Gpio *csPin_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::init(Bus *, Gpio *)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::reset(void)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::reset(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::setSampleRate(uint16_t sampleRate_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::setSampleRate(uint16_t)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::getTemperature(float *tempValue_p, ThermometricScale tempScale_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::getTemperature(float *, ThermometricScale)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::getTemperature(Int32Q16_t *tempQ16_p, ThermometricScale tempScale_p)
{
    // FIXME: Implement -> bool_t Mpu::getTemperature(Int32Q16_t *tempQ16_p, ThermometricScale tempScale_p)
    // Returns error
    this->_lastError = Error::UNDER_DEVELOPMENT;
    debugMessage(Error::UNDER_DEVELOPMENT);
    return false;
...
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_clearData(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::_init(void)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_init(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::_isInitialized(void)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_isInitialized(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::_reset(void)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_reset(void)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::_updateInterruptStatus(void)
{
    // Local variables
    uint8_t auxBuff;
...
bool_t Mpu::_read(Register regAddr_p, uint8_t *dataBuffer_p, uint16_t bytesToRead_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_read(Register, uint8_t *, uint16_t)", Debug::CodeIndex::MPU_MODULE);
...
bool_t Mpu::_write(Register regAddr_p, uint8_t *dataBuffer_p, uint16_t bytesToWrite_p)
{
    // Mark passage for debugging purpose
    debugMark("Mpu::_write(Register, uint8_t *, uint16_t)", Debug::CodeIndex::MPU_MODULE);

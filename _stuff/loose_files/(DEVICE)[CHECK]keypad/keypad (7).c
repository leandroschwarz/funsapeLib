 ...
 * File:			keypad.c
 * Module:			Matrix keypad interface
...
/* -----------------------------------------------------------------------------
 * Resets the keypad configuration.
 * -------------------------------------------------------------------------- */
...
keypadResult_t keypadSetPort(volatile keypadConfiguration_t *keypad, volatile uint8 *linesDDR,
        volatile uint8 *linesPORT, volatile uint8 *linesPIN, uint8 linesLSB, volatile uint8 *columnsDDR,
        volatile uint8 *columnsPORT, uint8 columnsLSB)
{
    ...
    for (i = 0; i < keypad->columns; i++) {					// For each column
        ...
        for (j = 0; j < keypad->lines; j++) {				// For each line
            if (isBitClr(aux, j)) {					// Tests if the key is pressed
                ...
                _delay_ms(keypad->debounceTime);		// Debounce time
            }

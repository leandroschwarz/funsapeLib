...
#include "lcd8f.h"
#if __LCD8F_H != 11
	#error Error 101 - Version mismatch on header and source code files (lcd8f).
#endif
...
lcdResult_t lcdSetDataPort(volatile lcdConfiguration_t * lcd, volatile uint8 * dataDDR, volatile uint8 * dataPORT, volatile uint8 * dataPIN)
{
	lcd->dataDDR = dataDDR;
